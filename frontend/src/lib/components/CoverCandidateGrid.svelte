...
{#if loading}
	<div class="flex items-center gap-3 py-8">
		...
		<span>{$_('missingCovers.loadingCandidates')}</span>
	</div>
{:else}
	{#if error}
		<Alert type="error">
	...
	{#if sorted.length === 0}
		<div class="text-sm text-base-content/60 py-4">{emptyMessage ?? $_('book.autoSearchNoCandidates')}</div>
	{:else}
		<div class="grid grid-cols-2 md:grid-cols-3 gap-3" class:pointer-events-none={disabled}>
			{#each sorted as candidate (resolutionKey(candidate))}
				...
				>
					<figure class="bg-base-200">
						<img
							src={candidate.url}
							alt={$_('book.autoSearchSourceLabel', { values: { source: candidate.source } })}
